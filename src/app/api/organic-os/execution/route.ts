...
export async function POST(request: Request) {
  ...
  const { workspaceId, workflowId, agentId, objective, strategy, budget, contextTokens } = body;
  ...
  const { plan, warnings } = svc.createPlan({ workspaceId, workflowId, agentId, objective, strategy, budget, contextTokens });

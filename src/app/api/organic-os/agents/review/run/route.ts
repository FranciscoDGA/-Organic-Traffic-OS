...
export async function POST(req: NextRequest) {
  try {
    ...
    let { draft_pack, evidence_pack, research_pack } = body;
    
    // Auto-pipeline
    if (!draft_pack || !evidence_pack || !research_pack) {
    ...
    const report = await revService.runReview({ blog_id, draft_pack, evidence_pack, research_pack, mode: body.mode || 'mock' });

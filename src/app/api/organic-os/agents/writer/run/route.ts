...
export async function POST(req: NextRequest) {
  try {
    ...
    let { research_pack, evidence_pack } = body;
    
    // Auto-pipeline if no packs provided
    if (!evidence_pack || !research_pack) {
    ...
    const report = await wService.runWriterDraft({ blog_id, research_pack, evidence_pack, mode: body.mode || 'mock' });

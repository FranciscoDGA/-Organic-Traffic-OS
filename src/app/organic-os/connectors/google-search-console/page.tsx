...
const TABS = ['Status', 'Sites', 'Consultas', 'Páginas', 'Logs'];

export default function GoogleSearchConsoleConnectorPanel() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await fetch('/api/organic-os/connectors/google-search-console/status');
  ...
  async function initializeAndAuth() {
    setLoading(true);
    try {
      const res = await fetch('/api/organic-os/connectors/google-search-console/connect', {
        method: 'POST',
  ...
  async function syncSite() {
    ...
    setSyncing(true);
    try {
      const res = await fetch('/api/organic-os/connectors/google-search-console/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function loadSites() {
    try {
      const res = await fetch('/api/organic-os/connectors/google-search-console/sites');
      const data = await res.json();
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '40px' }}>
        <div>
          ...
          <p style={{ color: '#64748b', marginTop: '8px', fontSize: '15px', margin: '8px 0 0' }}>
            Connector de dados reais de pesquisa. OAuth 2.0. Dados 100% desacoplados.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={card()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {isConnected && (
                <button onClick={async () => { await fetch('/api/organic-os/connectors/google-search-console/connect', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'disconnect' }) }); await fetchStatus(); }} style={{ ...btnDanger, fontSize: '12px', padding: '8px' }}>
                  Desconectar
                </button>
              )}
            </div>
          ...
          <div style={{ display: 'flex', gap: '4px', backgroundColor: '#0e1119', border: '1px solid #1d2133', borderRadius: '12px', padding: '4px', marginBottom: '16px' }}>
            {TABS.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{
                ...
                backgroundColor: activeTab === tab ? '#4285f4' : 'transparent',
          ...
          {activeTab === 'Status' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                {[
                  { label: 'Consultas Sincronizadas', value: status?.total_queries || 0, icon: '🔍' },
                  { label: 'Páginas Sincronizadas', value: status?.total_pages || 0, icon: '📄' },
              ...
              {lastSyncResult && (
                <div style={card({ borderLeft: '4px solid #14b8a6' })}>
                  <div style={{ fontSize: '12px', fontWeight: '700', color: '#14b8a6', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {sites.map((s: any, i: number) => (
                    <div key={i} style={{
                      ...
                      borderLeft: `3px solid ${selectedSite === s.siteUrl ? '#4285f4' : '#1d2133'}`,
                      cursor: 'pointer',
                    }} onClick={() => setSelectedSite(s.siteUrl)}>
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: '#e2e8f0' }}>{s.siteUrl}</div>
                        <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>{s.permissionLevel}</div>
                      </div>
                      {selectedSite === s.siteUrl && <span style={badge('#4285f4')}>SELECIONADO</span>}
                    </div>
                  ))}
                </div>
              )}
